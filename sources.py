...
def parse_ashby_payload(payload: dict[str, Any], company: str) -> list[dict[str, str]]:
    jobs: list[dict[str, str]] = []
    for item in payload.get("jobs", []) if isinstance(payload, dict) else []:
        if not isinstance(item, dict):
            continue
        location = item.get("location") or ""
        if item.get("isRemote") and "remote" not in clean_text(location).casefold():
...
def fetch_ashby_jobs(token: str, company: str) -> list[dict[str, str]]:
    url = f"https://api.ashbyhq.com/posting-api/job-board/{token}"
    response = SESSION.get(url, timeout=30)
    response.raise_for_status()
    return parse_ashby_payload(response.json(), company)


def parse_recruitee_payload(payload: dict[str, Any], company: str) -> list[dict[str, str]]:
    jobs: list[dict[str, str]] = []
    ...
    for item in items:
        ...
        if isinstance(location_data, dict):
            location = location_data.get("name") or location_data.get("city") or ""
        else:
            location = location_data
        if item.get("remote") and "remote" not in clean_text(location).casefold():
            location = f"{location}; Remote" if location else "Remote"
...
def fetch_recruitee_jobs(token: str, company: str) -> list[dict[str, str]]:
    url = f"https://{token}.recruitee.com/api/offers/"
    response = SESSION.get(url, timeout=30)
    response.raise_for_status()
    return parse_recruitee_payload(response.json(), company)


def parse_workable_payload(payload: Any, company: str) -> list[dict[str, str]]:
    ...
    for item in items:
        ...
        location_data = item.get("location") or {}
        if isinstance(location_data, dict):
            location = location_data.get("location_str") or ", ".join(
        ...
        jobs.append(
            _job(
                ...
                location,
                item.get("url") or item.get("shortlink") or item.get("application_url"),
                item.get("description") or item.get("description_html") or "",
                "workable",
            )
        )
    return jobs[:MAX_JOBS_PER_SOURCE]
...
def fetch_workable_jobs(token: str, company: str) -> list[dict[str, str]]:
    url = f"https://apply.workable.com/api/v3/accounts/{token}/jobs"
    response = SESSION.get(url, timeout=30)
    response.raise_for_status()
    return parse_workable_payload(response.json(), company)
...
# ---------------------------------------------------------------------------
# Additional sanctioned academic feeds and public remote-job APIs
# ---------------------------------------------------------------------------


def parse_mathjobs_html(html: str, company: str, base_url: str) -> list[dict[str, str]]:
    soup = BeautifulSoup(html, "lxml")
    jobs: list[dict[str, str]] = []
    current_institution = company
    for element in soup.find_all(["h3", "h4", "a"]):
        ...
        if "/jobs/" not in urlparse(full_url).path or full_url.rstrip("/") == base_url.rstrip("/"):
            continue
        card = _nearest_card_text(element)
        if not _discovery_relevant(title, card):
            continue
        jobs.append(_job(current_institution, title, "", full_url, card, "mathjobs"))
    return _unique_jobs(jobs)[:MAX_JOBS_PER_SOURCE]


def fetch_mathjobs_jobs(source: dict[str, Any], company: str) -> list[dict[str, str]]:
    url = str(source["url"])
    response = SESSION.get(url, timeout=30)
    response.raise_for_status()
    return parse_mathjobs_html(response.text, company, url)


def parse_jobrxiv_html(html: str, company: str, base_url: str) -> list[dict[str, str]]:
    soup = BeautifulSoup(html, "lxml")
    jobs: list[dict[str, str]] = []
    ...
    for anchor in soup.select('a[href*="/job/"]'):
        ...
        if not _discovery_relevant(title, card):
            continue
        listed_company, location = _parse_pipe_metadata(card, title)
        jobs.append(
            _job(
                listed_company or company,
                title,
                location,
                url,
                card,
...
def fetch_jobrxiv_jobs(source: dict[str, Any], company: str) -> list[dict[str, str]]:
    urls = source.get("urls") or [source["url"]]
    jobs: list[dict[str, str]] = []
    for url in urls:
        response = SESSION.get(str(url), timeout=30)
        response.raise_for_status()
        jobs.extend(parse_jobrxiv_html(response.text, company, str(url)))
    return _unique_jobs(jobs)[:MAX_JOBS_PER_SOURCE]


def parse_jobicy_payload(payload: dict[str, Any], company: str) -> list[dict[str, str]]:
    jobs: list[dict[str, str]] = []
    for item in payload.get("jobs", []) if isinstance(payload, dict) else []:
        if not isinstance(item, dict):
            continue
        jobs.append(
            _job(
                item.get("companyName") or company,
                item.get("jobTitle"),
                item.get("jobGeo"),
                item.get("url"),
...
def fetch_jobicy_jobs(source: dict[str, Any], company: str) -> list[dict[str, str]]:
    queries = source.get("queries") or [""]
    jobs: list[dict[str, str]] = []
    for query in queries:
        params = {"count": int(source.get("count", 100))}
        if query:
            params["tag"] = query
        response = SESSION.get(str(source["url"]), params=params, timeout=30)
        response.raise_for_status()
        jobs.extend(parse_jobicy_payload(response.json(), company))
    return _unique_jobs(jobs)[:MAX_JOBS_PER_SOURCE]


def parse_himalayas_payload(payload: dict[str, Any], company: str) -> list[dict[str, str]]:
    items = []
    if isinstance(payload, dict):
    ...
    for item in items if isinstance(items, list) else []:
        ...
        ) or item.get("companyName") or company
        location = (
        ...
        if isinstance(location, list):
            location = ", ".join(str(value) for value in location)
        jobs.append(
            _job(
                listed_company,
                item.get("title"),
...
def fetch_himalayas_jobs(source: dict[str, Any], company: str) -> list[dict[str, str]]:
    queries = source.get("queries") or [""]
    jobs: list[dict[str, str]] = []
    for query in queries:
        params: dict[str, Any] = {"sort": "recent", "page": 1}
        if query:
            params["q"] = query
        response = SESSION.get(str(source["url"]), params=params, timeout=30)
        response.raise_for_status()
        jobs.extend(parse_himalayas_payload(response.json(), company))
    return _unique_jobs(jobs)[:MAX_JOBS_PER_SOURCE]


def parse_remotive_payload(payload: dict[str, Any], company: str) -> list[dict[str, str]]:
    jobs: list[dict[str, str]] = []
    for item in payload.get("jobs", []) if isinstance(payload, dict) else []:
        if not isinstance(item, dict):
            continue
...
def fetch_remotive_jobs(source: dict[str, Any], company: str) -> list[dict[str, str]]:
    response = SESSION.get(str(source["url"]), timeout=30)
    response.raise_for_status()
    return parse_remotive_payload(response.json(), company)
...
def fetch_jobs(source: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if source_type == "ashby":
        return fetch_ashby_jobs(source["token"], company)
    if source_type == "recruitee":
        return fetch_recruitee_jobs(source["token"], company)
    if source_type == "workable":
        return fetch_workable_jobs(source["token"], company)
    ...
    if source_type == "mathjobs":
        return fetch_mathjobs_jobs(source, company)
    if source_type == "jobrxiv":
        return fetch_jobrxiv_jobs(source, company)
    if source_type == "jobicy":
        return fetch_jobicy_jobs(source, company)
    if source_type == "himalayas":
        return fetch_himalayas_jobs(source, company)
    if source_type == "remotive":
        return fetch_remotive_jobs(source, company)
